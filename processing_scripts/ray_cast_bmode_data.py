...
def generate_raycasted_db(db_path, output_dir):
    for split in ["all_data"]:
        ...
        for (image_name, label_name) in zip(us_images, us_labels):
            ...
            label_name_expected = image_name.replace(suffix, "label_" + suffix)

            labelpath = os.path.join(split_path, label_name)

            assert label_name == label_name_expected, "non correct label"
            assert os.path.exists(labelpath), "Inexisting label path"
...
generate_raycasted_db(db_path="E:/NAS/jane_project/unet_data/segmentation_network_data/full_labels",
                       output_dir="E:/NAS/jane_project/unet_data/segmentation_network_data/ray_casted_labels")
