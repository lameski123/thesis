...
def get_image_points(image, T, image_width, image_height):
    ...
    image_spacing = [image_width/image.shape[0], image_height/image.shape[1]]
...
def get_point_cloud(data_list, transform_list, image_width, image_height):
    ...
    overall_pc = np.concatenate(pc_list, axis=1)
...
def reorder_ts_data(ts_path):
    image_file_names = os.listdir(ts_path)
...
def main(unet_segmentation_path, us_sweep_dir, save_dir):
    ...
    for spine_id in os.listdir(unet_segmentation_path):
        ...
        for ts_name in ts_list:
            us_sweep, = imfusion.open( os.path.join(us_sweep_dir, spine_id, ts_name))
            ...
            np.savetxt(os.path.join(save_folder, "raycasted" + ts_name + ".txt"), us_point_cloud)
